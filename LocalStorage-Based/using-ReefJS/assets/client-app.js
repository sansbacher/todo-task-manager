...
let todoStore = new Reef.Store({						// A Reef data store holds the main array for the todo list (could also use getters and setters to limit access)
	data: {
		todoItems: []
	}
})
...
$todoList.addEventListener('render', () => {													// Reef 'render' event triggered when updating the DOM
		localStorage.setItem(localStorageKey, JSON.stringify(todoStore.data.todoItems)) 		// Preserve state by saving todoItems to LocalStorage
})

document.getElementById('btn-clear-all').addEventListener('click', () => {						// Clear the whole List button
	todoStore.data.todoItems = []
})
...
document.addEventListener('DOMContentLoaded', () => {											// Initialization Handler for when DOM is complete
	...
	if (existingContent) {
		todoStore.data.todoItems = JSON.parse(existingContent)			// LocalStorage can only save String data
	}
...
let todoApp = new Reef($todoList, {					// Can also pass '#todo-list' and Reef will select it
	store: todoStore,								// Use the data store instead of a direct data: object
...
function addTodo(text) {
	...
	todoStore.data.todoItems.push(todo)
}

function checkTodo(id) {
	const index = todoStore.data.todoItems.findIndex( (item) => item.id == id )

	todoStore.data.todoItems[index].completed = !todoStore.data.todoItems[index].completed			// toggle the completed property
}

function deleteTodo(id) {
	todoStore.data.todoItems = todoStore.data.todoItems.filter( (item) => item.id != id )			// Remove it from the main array
}

function editTodo(id) {
	const index = todoStore.data.todoItems.findIndex( (item) => item.id == id )

	let input = window.prompt("Edit Todo item", todoStore.data.todoItems[index].description)
	let inputValue = input ? stripHTML(input).trim() : ''			// NO need for sanitizeHTML() as Reef sanitizes HTML, but I'd rather strip it off
	if (inputValue != '' && inputValue != todoStore.data.todoItems[index].description) {
		todoStore.data.todoItems[index].description = inputValue
